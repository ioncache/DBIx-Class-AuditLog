...
$schema->populate('UserForceaudit', [
[qw/name email phone/],
...
$schema->txn_do(
    sub {
        $test_user
            = $schema->resultset('UserForceaudit')->find( { name => "JohnSample" } )
            ->update(
            {   name  => 'JaneSample',
                phone => '123-456-7890',
            }
...
is $change->old_value, $change->new_value, "old_value (email) equals new_value (email)";
