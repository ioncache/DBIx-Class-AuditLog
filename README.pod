...
If you are using a DBIx::Class generated schema, and don't want to modify the column defintions directly, you can ad the following to the editable portion of the Result Class file:
...
To deploy and AuditLog schema, load your main schema, and then run the deploy command on the audit_log_schema:
...
=head1 ACCESSING THE AUDIT LOG SCHEMA

The Audit Log schema ca be accessed from your main schema by calling the audit_log_schema method:
